...
from src.radiative_shift.dyson_solvers.param import LBAR, GAMMA, C, OM
import numpy as np
...
n0 = DEN
om = OM
omd = om + 11.69 * n0 * GAMMA
kd = omd / C
density = n0 * kd ** 3

noa = int(l * density * 3.14 * r ** 2)
print(noa)

model = HexagonModel(l, r, density)
model.rotate_model(np.pi/6)

sigma_v = MarkovianSigmaMatrixForV(model, kd)
resolvent = sigma_v.get_resolvent_for_v(om, kd)
...
for i in range(len(x)):
    ...
    s = sigma_v.get_sigma_outside(model, resolvent, kd, m0, m)
...
ax.plot(np.linspace(1, 5, len(vpgreen_y)), vpgreen_y, color='green', label='Fundamental mode and natural decay diff., PRA 97, 023827')
